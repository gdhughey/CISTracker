...
function renderBatch() {
  ...
  for (let i = renderShown; i < end; i++) {
    ...
    tr.innerHTML = `
      ...
      <td>${statusCell}</td>
      <td>${selectionMode ? '' : actionCell}</td>
    `;
    frag.appendChild(tr);
  }
  renderShown = end;
  ...
  if (renderShown < renderVisible.length) {
    ...
    sentinel.innerHTML = `<td colspan="${colspan}" style="text-align:center;padding:14px;color:var(--text-muted);font-family:var(--mono);font-size:11px">↓ ${renderVisible.length - renderShown} more — click or scroll</td>`;
...
function toggleSelectItem(id, checked) {
  ...
  const allCheck = document.getElementById('selectAllCheck');
  if (allCheck) {
    allCheck.checked = renderVisible.length > 0 && renderVisible.every(i => selectedIds.has(i.id));
  }
  updateActionBar();
}
...
function selectByFilter(field, value) {
  ...
  renderItems();
  updateActionBar();
  toast(`Selected all in ${field}: ${value}`, 'success');
}

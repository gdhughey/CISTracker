...
async function sendPasswordReset(to, username, tempPassword) {
  return send({
    to,
    subject: 'CIS Tracker: Your password has been reset',
    html: `
      <h2>Password Reset</h2>
      <p>Hi <strong>${username}</strong>,</p>
      <p>An admin has reset your CIS Tracker account password.</p>
      ...
      <p style="color:#888;font-size:12px">&mdash; CIS Tracker</p>
    `,
    text: `Hi ${username}, your CIS Tracker password has been reset. Temporary password: ${tempPassword}  — You must change it on next login.`,
  });
}
...
async function sendNewAccount(to, username, tempPassword) {
  return send({
    to,
    subject: 'CIS Tracker: Your account has been created',
    html: `
      <h2>Welcome to CIS Tracker</h2>
      <p>Hi <strong>${username}</strong>,</p>
      <p>An account has been created for you on the CIS Tracker.</p>
      <p>Your temporary password is: <code style="font-size:16px;background:#1e293b;color:#38bdf8;padding:4px 10px;border-radius:4px">${tempPassword}</code></p>
      <p>You will be required to change this password on your first login.</p>
      <p style="color:#888;font-size:12px">&mdash; CIS Tracker</p>
    `,
    text: `Hi ${username}, a CIS Tracker account was created for you. Temporary password: ${tempPassword}  — You must change it on first login.`,
  });
}
...
async function sendOverdueReminder(to, username, items) {
  ...
  return send({
    to,
    subject: `CIS Tracker: ${items.length} item(s) overdue`,
    html: `
      <h2>Equipment Overdue Reminder</h2>
      <p>Hi <strong>${username}</strong>,</p>
      <p>The following equipment needs to be returned:</p>
      <ul>${itemList}</ul>
      <p>Please return these items to the CIS Tracker as soon as possible.</p>
      <p style="color:#888;font-size:12px">&mdash; CIS Tracker</p>
    `,
    text: `Hi ${username}, you have ${items.length} overdue item(s). Please return them to the CIS Tracker.`,
  });
}
...
async function sendAdminAlert(to, overdueItems) {
  ...
  return send({
    to,
    subject: `CIS Tracker Alert: ${overdueItems.length} item(s) overdue 5+ days`,
    html: `
      ...
      <p style="color:#888;font-size:12px">&mdash; CIS Tracker</p>
    `,
    text: `CIS Tracker: ${overdueItems.length} item(s) overdue 5+ days. Check the admin panel.`,
  });
}
